# ===============================================================
# Clash Detection - ICM & WS Pro (refactor step 1)
...
def log_warning(message)
  WSApplication.log(message)
rescue NoMethodError, RuntimeError
  puts(message)
end

def safe_optional(context = nil, log: false)
  yield
rescue NoMethodError, RuntimeError, ArgumentError, TypeError => e
  # Optional API methods/fields vary across products and versions.
  log_warning("Warning: #{context} (#{e.class}: #{e.message})") if log && context
  nil
end

def safe_best_effort(context)
  yield
rescue StandardError => e
  log_warning("Warning: #{context} (#{e.class}: #{e.message})")
  nil
end

def ask_float(prompt, default)
  s = WSApplication.input_box(prompt, "", "").to_s.strip
  return default if s.empty?
  Float(s)
rescue NoMethodError, RuntimeError, ArgumentError, TypeError => e
  prompt_hint = prompt.to_s.lines.first.to_s.strip
  log_warning("Warning: using default #{default} for '#{prompt_hint}' (#{e.class}: #{e.message})")
  default
end
...
THRESHOLD_M = [THRESHOLD_M, 0.0].max
XY_TOL_M    = [XY_TOL_M, 0.0].max
...
def safe_get(ro, names)
  names.each do |n|
    x = safe_optional { ro[n] }
    return x unless x.nil?
  end
  nil
end

def safe_set(ro, names, value)
  names.each do |n|
    written = safe_optional { ro[n] = value; true }
...
def set_progress(p)
  safe_optional { WSApplication.progress_value = p }
end
...
def pipe_level_at_param(link, r)
  ...
  if us == 0.0 && ds == 0.0
    us_node = safe_optional { link.us_node }
    ds_node = safe_optional { link.ds_node }
    if us_node && ds_node
...
def link_seg_and_len(link)
  us = link.us_node
  ds = link.ds_node
  raise RuntimeError, "Link #{link.id}: missing US/DS node" if us.nil? || ds.nil?
...
def pick_table(net)
  %w[
    ...
    ros = safe_optional { net.row_objects(t) }
...
def pick_point_table(net)
  %w[
    ...
    test = safe_optional { net.row_objects(t) }
    return t if test && test.respond_to?(:length)
  end
  nil
end
...
def add_label_point(net, pt_table, x, y, text)
  ...
  p = safe_optional("Unable to create row object in #{pt_table}", log: true) { net.new_row_object(pt_table) }
  return if p.nil?

  safe_set(p, %w[X x], x)
  safe_set(p, %w[Y y], y)
  safe_set(p, %w[label name text comment description user_text user_text_1 user_text_2], "[CLR] #{text}")
  safe_optional("Unable to write label point", log: true) { p.write }
end

def delete_old_labels(net, pt_table)
  return unless pt_table

  ros = safe_optional("Unable to read point table #{pt_table}", log: true) { net.row_objects(pt_table) }
  return if ros.nil?

  ros.each do |r|
    labelish = safe_get(r, %w[label name text comment description user_text user_text_1 user_text_2])
    next unless labelish && labelish.to_s.start_with?("[CLR]")
    safe_optional("Unable to delete an old clash label", log: true) { r.delete }
  end
end
...
def commit_selection_list(link_table, ids)
  # Some contexts/products do not expose Selection list APIs.
  return :unavailable unless defined?(Selection)

  sel = Selection.new
  ids.each { |id| sel.add(link_table, id) }
  sel.commit("Clash_Selection_#{Time.now.strftime('%Y%m%d_%H%M')}")
  :ok
rescue NameError, NoMethodError, RuntimeError, ArgumentError, TypeError => e
  log_warning("Warning: Selection list commit failed; using direct selection (#{e.class}: #{e.message})")
  :failed
end

def apply_direct_selection(net, link_table, ids)
  safe_optional { net.clear_selection }
  safe_optional { net.transaction_begin }
  begin
    ids.each do |id|
      ro = safe_optional { net.row_object(link_table, id) }
      next if ro.nil?
      safe_optional { ro.selected = true }
      safe_optional { ro.write }
    end
  ensure
    safe_optional { net.transaction_commit }
  end
end

# ---------------- Main ----------------
net = WSApplication.current_network
raise RuntimeError, "No network is currently open." if net.nil?
picked = pick_table(net)
raise RuntimeError, "No usable link table found." if picked.nil?
link_table, all_links = picked

sel_ids = []
all_links.each do |ro|
  selected = safe_optional { ro.selected }
  sel_ids << ro.id if selected
end

links = []
if sel_ids.any?
  sel_set = Set.new(sel_ids)
  all_links.each do |ro|
    links << ro if sel_set.include?(ro.id)
  end
else
  all_links.each { |ro| links << ro }
end
raise RuntimeError, "No links to process (selection empty?)" if links.empty?
...
links.each do |l|
  seg_len = safe_optional("Skipping link #{l.id}: invalid geometry", log: true) { link_seg_and_len(l) }
  next if seg_len.nil?

  seg, len = seg_len
  next if len <= 0.0

  idx = valid_links.length
  valid_links << l
...
raise RuntimeError, "No valid links with usable geometry found." if valid_links.empty?
...
safe_optional { WSApplication.long_running_operation_begin }
begin
  segs.each_index do |i|
    l1 = valid_links[i]
    seg1 = segs[i]
    ...
    (candidates[i] || []).each do |j|
      ...
      unless INCLUDE_SHARED_NODES
        ids1 = safe_optional { [l1.us_node_id, l1.ds_node_id] } || []
        ids2 = safe_optional { [l2.us_node_id, l2.ds_node_id] } || []
      ...
      if d_xy <= XY_TOL_M
        ...
        results << { l1: l1.id, l2: l2.id, clr: clearance, x: ix, y: iy } if clash
      end
  ...
  safe_optional { WSApplication.long_running_operation_end }
end
...
results.each_with_index do |r, k|
  puts "%2d. %-16s x %-16s | clearance = % .6f m" % [k + 1, r[:l1], r[:l2], r[:clr]]
end

if results.any?
  ids = results.flat_map { |r| [r[:l1], r[:l2]] }.uniq
  selection_status = commit_selection_list(link_table, ids)
  if selection_status == :ok
    puts "Selection list saved."
  else
    apply_direct_selection(net, link_table, ids)
    puts "Direct selection applied to clash links."
  end
end
...
if results.any?
  path = safe_best_effort("Unable to write CSV report") do
  ...
  puts "CSV saved: #{path}" unless path.nil?
end
