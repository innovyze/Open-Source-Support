...
# Method to import controls from a CSV file
def import_ctrl_hydraulics(open_net, csv_path)
  ...
  open_net.row_objects('hw_pump').each do |ro|
    ...
    ro.switch_on_level = row["ON"]
    ro.switch_off_level = row["OFF"]
...
# Method to resolve the control set for a given scenario
def resolve_ctrl_set(scenario_id, scenario_data)
  current_scenario = scenario_data[scenario_id]
  return "BASE" unless current_scenario # Default to "BASE" if scenario is not found

  ctrl_set = current_scenario["CTRL_SET"].to_s.strip
  parent_scenario = current_scenario["PARENT"].to_s.strip

  # Traverse up the parent chain if CTRL_SET is blank
  while ctrl_set.empty? && !parent_scenario.empty?
    parent_data = scenario_data[parent_scenario]
    break unless parent_data

    ctrl_set = parent_data["CTRL_SET"].to_s.strip
    parent_scenario = parent_data["PARENT"].to_s.strip
  end

  ctrl_set = "BASE" if ctrl_set.empty?
  ctrl_set
end
...
scenario_data.each do |scenario_id, data|
  ...
  ctrl_set = resolve_ctrl_set(scenario_id, scenario_data)
  scenario_paths[scenario_id] = File.join(iedb_folder_path, "Control", ctrl_set, "CONTROL.CSV")
  ctrl_sets[scenario_id] = ctrl_set
end
...
open_net.scenarios do |scenario|
  ...
  ctrl_set = ctrl_sets[scenario_id]

  # Check if the csv_path is valid
  if csv_path.nil? || !File.exist?(csv_path)
    puts "\nWarning: No valid CONTROL.CSV file path for scenario '#{scenario_id}'. Skipping..."
    next
  end

  puts "\nImporting CONTROL to scenario '#{scenario_id}' from InfoSewer Control Set '#{ctrl_set}'"
  open_net.transaction_begin
  begin
  import_ctrl_hydraulics(open_net, csv_path)
  open_net.transaction_commit
  rescue => e
    open_net.transaction_rollback
    puts "Error importing control data for scenario '#{scenario_id}': #{e.message}"
  end
...
puts "\nFinished import of InfoSewer CONTROL scenario data to InfoWorks ICM"