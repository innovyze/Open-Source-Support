# Trace all flow paths from terminal upstream nodes to selected downstream node(s)
# 
# Instructions:
# 1. Select one or more downstream nodes (hold Ctrl to select multiple)
# 2. Run this script
# 3. The script will find all terminal upstream nodes
# 4. For each terminal node, traces the path to each selected downstream node
# 5. Each path will be saved as a separate Selection List
#
# Selection lists will be named: "Path_<upstream_node_id>_to_<downstream_node_id>"
...
class PathTracer
  ...
  # Only follows downstream links (flow direction)
  def find_path(start_node, end_node)
    ...
    while working.size > 0
      ...
      # Only follow downstream links (flow direction)
      current.ds_links.each do |l|
        node = l.ds_node
    ...
    begin
      terminal_nodes.each do |terminal_node|
        downstream_nodes.each do |downstream_node|
          ...
          puts "Processing path #{total_paths}: #{terminal_node.node_id} -> #{downstream_node.node_id}"
          ...
          # Find path
          found = find_path(terminal_node, downstream_node)
          ...
          base_name = "Path_#{terminal_node.node_id}_to_#{downstream_node.node_id}"
