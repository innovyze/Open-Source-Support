...
header_fields = NodeResultsDataFields.map { |field| field.ljust(10) }.join(' ')
puts ";#{'Node ID'.ljust(10)} #{'Time'.ljust(10)} #{header_fields} hec22_spread hec22_eff"
...
net.each_selected do |sel|
  begin
    ...
    ts.each_with_index do |timestep, index|
      ...
      hec22_spread = depnod_value**0.42
      icm_spread = all_field_results[NodeResultsDataFields.index('GTTRSPRD')].to_f rescue 0.0

      if icm_spread > 0.01
        if hec22_spread > 0.0
          #hec22_eff = 1.0 - ((1.0 - 0.457 / hec22_spread)**1.8)
          hec22_eff = hec22_spread**1.8
        else
          hec22_eff = 0.0 # or appropriate handling for the case when hec22_spread is 0
        end
      else
        hec22_eff = 1.0
      end
      ...
      puts "#{sel.id.ljust(10)} #{hours.to_i.to_s.rjust(2, '0')}:#{format('%02d', minutes)}:#{format('%02d', seconds)} " +
           "#{all_field_results.map { |result| result.ljust(10) }.join(' ')} #{'%.4f' % hec22_spread} #{'%.4f' % hec22_eff} #{'%.4f' % hec22_eff_diff}"
