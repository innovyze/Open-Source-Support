...
[
  ...
  ['Convert MGD to GPM', 'Boolean', false]
], false


use_usa_units = parameters[0]
use_si_units = parameters[1]
include_peak_flow = parameters[2]
convert_mgd_to_gpm = parameters[3]
  ...
  begin
    ...
    if results.size == ts.size
      ...
      results.each do |result|
        ...
        # Calculate inflow as peak_gpm divided by the length of the link
        #inflow = peak_gpm / sel.length

        # Add the inflow to the selected link ID
        #sel.inflow = inflow
        
        # Print the value for each element with link id
        puts format("Link ID: %-10s | ICM Peaking Flow: %-10s | Total Flow GPM: %-10s", sel.id, '%.2f' % val, '%.2f' % peak_gpm)
      end
...
#==============================================================================
# Define database fields for ICM network subcatchments
database_fields = [
  'trade_flow',
  'base_flow',
...
cn.clear_selection
puts "Scenario     : #{cn.current_scenario}"
