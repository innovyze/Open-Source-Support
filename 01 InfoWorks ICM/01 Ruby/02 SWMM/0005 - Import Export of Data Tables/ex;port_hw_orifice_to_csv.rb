...
FIELDS_TO_EXPORT = [
  ...
  ['Include Link Type', :link_type, true, 'Link Type'], # Should be ORIFICE
  ...
  ['Include Invert Level', :invert, true, 'Invert Level'],
  ['Include Diameter', :diameter, true, 'Diameter'],
  ['Include Opening Type', :opening_type, false, 'Opening Type'], # e.g., CIRCULAR, RECTANGULAR
  ...
  ['Include Limiting Discharge', :limiting_discharge, false, 'Limiting Discharge'],
...
# Uncomment the following lines to print available methods for the first 'hw_orifice' object.
...
begin
  CSV.open(file_path, "w") do |csv|
    ...
    row_objects_iterator.each do |orifice_obj|
      ...
      us_node = orifice_obj.respond_to?(:us_node_id) ? orifice_obj.us_node_id.to_s : 'N/A'
      suffix = orifice_obj.respond_to?(:link_suffix) ? orifice_obj.link_suffix.to_s : 'N/A'
      current_orifice_id_for_log = "#{us_node}.#{suffix}"
      ...
      if orifice_obj && orifice_obj.respond_to?(:selected) && orifice_obj.selected
        ...
        selected_fields_config.each do |field_info|
          ...
          begin
            ...
            puts "Warning: Attribute (method) ':#{attr_sym}' (for field '#{field_info[:original_label]}') not found for selected 'hw_orifice' '#{current_orifice_id_for_log}'."
...
puts "\nScript for 'hw_orifice' finished at #{end_time}"
...
if file_exists_and_has_data
  ...
  WSApplication.message_box("No 'hw_orifice' objects were selected for export. The CSV file was not created or was empty (and thus deleted).",'OK',nil,false)
else
  WSApplication.message_box("Export for 'hw_orifice' did not complete as expected. No orifices written. Check console messages. The CSV file may not exist or is empty.",'OK',nil,false)
end
