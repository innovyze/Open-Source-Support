    ...
    result_field_names.each do |res_field_name|
      begin
        ...
        if rs_size == ts_size
          ...
          if ['LATERAL_INFLOW', 'TOTAL_INFLOW'].include?(res_field_name)
          puts "Node: #{'%-16s' % sel.node_id} | Field: #{'%-12s' % res_field_name} | Sum: #{'%15.4f' % total_integrated_over_time} | Mean: #{'%15.4f' % mean_value} | Max: #{'%15.4f' % max_value} | Min: #{'%15.4f' % min_value} | Steps: #{'%15d' % count}"
          else
          puts "Node: #{'%-16s' % sel.node_id} | Field: #{'%-12s' % res_field_name} | End: #{'%15.4f' % total_integrated_over_time} | Mean: #{'%15.4f' % mean_value} | Max: #{'%15.4f' % max_value} | Min: #{'%15.4f' % min_value} | Steps: #{'%15d' % count}"
          end 
        end
