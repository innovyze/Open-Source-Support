...
puts '  Subcatchment                 mm         mm         mm         mm         mm         mm         mm    10^6 ltr      LPS                    mm         mm         mm         mm         mm         mm         mm         mm         mm         mm'
puts '  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------'
...
aggregated_data.each do |sub_id, data|
  ...
  if data['rainfall']
    stats = data['rainfall']
    output_line += " #{'%*.4f' % [field_width, stats[:sum]]}"
  end
  if data['rainfall']
    stats = data['rainfall']
    output_line += " #{'%*.4f' % [field_width, stats[:min]]}"
  end
  if data['evaprate']
    stats = data['evaprate']
    output_line += " #{'%*.4f' % [field_width, stats[:sum]]}"
  ...
  if data['runoff']
    stats = data['runoff']
    second_runoff = stats[:sum] / 1000.0
    third_runoff = stats[:max] * 1000.0
