...
net.each_selected do |sel|
  begin
    ...
    # Check if full_flow or diameter is nil
    if full_flow.nil? || diameter.nil?
      puts "Link: #{'%-12s' % sel.id} has nil values for diameter or full flow."
    else
      puts "Link: #{'%-12s' % sel.id} | Diameter: #{'%12.3f' % diameter} | Full Flow: #{'%11.3f' % full_flow}"
    end
