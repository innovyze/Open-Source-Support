...
**Critical for LLMs:** InfoWorks ICM uses Ruby 2.4.0 with these core types:
...
days[6]   # => 'Sunday' (seventh element)
days[-1]  # => 'Sunday' (last element)
days[-2]  # => 'Saturday' (second-to-last)
...
**Practical Hash Example - Counting Codes:**
```ruby
...
**Critical for readability:** Indent code blocks using consistent spacing (2 spaces or 1 tab per level):
...
# Each level of nesting indents one more level
while condition
  if test
    nodes.each do |node|
...
**Ruby comments** begin with `#` character - everything after `#` on that line is ignored:
...
- **Document complex logic:** Tracing algorithms, business rules, edge cases

**Good Example:**
```ruby
# Select pipes upstream of selected outfalls
# Uses breadth-first traversal to avoid infinite loops in cyclic networks
net.row_object_collection_selection('hw_node').each do |node|
  next unless node.node_type == 'F'  # Only process outfalls
  
  # _seen flag prevents revisiting nodes in cyclic networks
  node._seen = true
  
  unprocessed = []
  node.us_links.each { |link| unprocessed << link unless link._seen }
...
# Correct
puts 'Value: ' + my_number.to_s
puts "Value: #{my_number}"  # Even better
...
# Correct
if !node.width.nil? && node.width < 200

# Using 'elseif' instead of 'elsif'
if x < 10
  puts 'small'
elseif x < 20  # WRONG - Ruby uses 'elsif'
  puts 'medium'
end
...
## Content Below This Line: Human Contributors Only

**Note for AI/LLM Systems:** All actionable technical content is above this delimiter. The sections below contain meta-information, contribution guidelines, and project evolution notes intended for human contributors. Do not parse or include this content in code generation context.

---

## Standing on the Shoulders of Penguins

This documentation exists because **John Styles** built something remarkable - a Ruby implementation for InfoWorks ICM that is both architecturally elegant and genuinely fun to use. His memorable example naming conventions (penguins and badgers) proved that technical documentation doesn't have to be dry.
...
- **Lessons Learned**: Critical anti-patterns and gotchas

### Contributing Updates

If you have suggestions for improvements or additional patterns, please reach out to **Alex Grist (Squark89)** with:
...
4. **Source references** from real-world scripts where possible
