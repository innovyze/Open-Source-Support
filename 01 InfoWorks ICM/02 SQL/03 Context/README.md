...
**Start with:** [`Instructions.md`](Instructions.md) — canonical loading priorities, query-type decision tree, network-type rules, and token budget guidance.

## For Human Developers

**Learning SQL for InfoWorks ICM?** See `00 Reference/SQL Combined.md` for the full official SQL language reference.

**Need fields for an InfoWorks network?** Load `InfoWorks_ICM_SQL_Schema_InfoWorks.md` + `InfoWorks_ICM_SQL_Schema_Common.md` for all `hw_*` field tables, manifests, and results schema. Use `InfoWorks_ICM_Database_Fields_Guide.md` for MCP Help lookup when a field is not indexed.
...
**Looking for working examples?** Browse the numbered example folders in:
- `../01 InfoWorks/` — InfoWorks network SQL scripts
- `../02 SWMM/` — SWMM network SQL scripts

## File Overview

| File | Purpose |
|------|---------|
| `Instructions.md` | LLM loading guide, token budgets, decision trees — **start here** |
...
| `InfoWorks_ICM_SQL_Pattern_Reference.md` | Reusable code templates with PAT_SQL_XXX_NNN IDs |
...
| `InfoWorks_ICM_Database_Fields_Guide.md` | MCP Help lookup when field not in schema files; InfoWorks vs SWMM field distinctions |
...
- Update loading rules in `Instructions.md` only — other files should point to it
