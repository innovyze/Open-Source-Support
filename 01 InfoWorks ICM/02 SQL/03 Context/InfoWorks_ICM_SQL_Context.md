...
- `cctv_surveys.field` - Access CCTV survey data from pipe (InfoAsset Manager)
- `details.field` - Access blob table details (observations, defects, etc.)
...
Spatial operations are unique to InfoWorks ICM:
...
### Node Fields (InfoWorks ICM)
- `node_id`, `x`, `y`, `ground_level`, `node_type`
- `flood_type`, `chamber_floor_level`, `shaft_area`
- `user_text_1` through `user_text_10`
- `user_number_1` through `user_number_10`

### Node/Manhole Fields (InfoAsset Manager)
- `node_id`, `x`, `y`, `cover_level`, `chamber_floor_level`
- `node_type`, `location`, `asset_id`
- `us_links`, `ds_links` (navigation)

### Link Fields (Conduit, Pipe)
- `us_node_id`, `ds_node_id`, `link_suffix`
- `conduit_length`, `conduit_width`, `conduit_height` (ICM)
- `length`, `pipe_width`, `pipe_height` (InfoAsset)
- `us_invert`, `ds_invert`, `shape`, `system_type`
- `asset_id`, `link_type`, `material` (InfoAsset: `pipe_material`)

### Subcatchment Fields
- `subcatchment_id`, `node_id`, `contributing_area`
- `area_measurement_type`, `area_percent_1`, `area_absolute_1`
- `land_use_id`, `runoff_index`, `trade_profile`

### Survey Fields (InfoAsset Manager)
- `id`, `when_surveyed`, `surveyed_by`
- `structural_grade`, `service_grade` (CCTV)
- `details.code`, `details.distance`, `details.remarks` (blob table)
- `oid` (unique object identifier)

### Results Fields (InfoWorks ICM)
- `tsr.timestep_no`, `tsr.timesteps` (max timesteps)
- `tsr.ds_depth`, `tsr.us_depth`, `tsr.ds_flow`, `tsr.us_flow`
- `tsr.us_vel`, `tsr.ds_vel`
...
- [InfoWorks ICM Help - Using SQL](https://help.autodesk.com/view/IWICMS/2024/ENU/?guid=GUID-9C3E4534-B861-4525-920D-D59BE9FEDC6F)