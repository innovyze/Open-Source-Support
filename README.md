...
Exchange scripts will sometimes be packaged alongside an `exchange.bat` file. This encapsulates a set of commands that provide the `*.rb` script to the Exchange executable in a specific Workgroup Client version. It allows a degree of customisation - see an example below:
```bat
@ECHO OFF
SET version=2021.1
SET bit=64
