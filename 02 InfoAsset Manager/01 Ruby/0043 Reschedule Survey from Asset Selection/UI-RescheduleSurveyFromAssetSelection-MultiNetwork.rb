...
# Script: UI-RescheduleSurveyFromAssetSelection-MultiNetwork.rb
# Purpose: Multi-network variant of UI-RescheduleSurveyFromAssetSelection.rb.
#          Supports Collection (CAMS), Distribution (WAMS), and Asset (AMS)
#          networks. For each selected asset, copies the most recent completed
#          survey of a chosen type and creates a new planned survey with ID
#          {asset_id}-{date_planned}.
...
NETWORK_PROFILES = {
  cams: {
    label: 'Collection (CAMS)',
    prefix: 'cams_',
    user_defined_prefix: 'cams__',
    pipe_table: 'cams_pipe',
    flood_defence_survey: 'cams_flood_defence_survey',
    default_survey: 'cams_manhole_survey'
  },
  wams: {
    label: 'Distribution (WAMS)',
    prefix: 'wams_',
    user_defined_prefix: 'wams__',
    pipe_table: 'wams_pipe',
    flood_defence_survey: nil,
    default_survey: 'wams_manhole_survey'
  },
  ams: {
    label: 'Asset (AMS)',
    prefix: 'ams_',
    user_defined_prefix: 'ams__',
    pipe_table: nil,
    flood_defence_survey: nil,
    default_survey: nil
  }
...
KNOWN_SURVEY_TABLES_BY_PROFILE = {
  cams: %w[
    cams_cctv_survey
  ...
  wams: %w[
    ...
    wams_mon_survey
    wams_pipe_sample
    wams_pump_station_survey
    wams_water_quality_survey
  ],
  ams: []
}.freeze
...
# Legacy display-name aliases for flood-defence surveys and older CAMS asset_type values.
CAMS_LEGACY_ASSET_TYPE_ALIASES = {
...
LEGACY_ASSET_TYPE_ALIASES_BY_PROFILE = {
  cams: CAMS_LEGACY_ASSET_TYPE_ALIASES,
  wams: {
    'node' => 'wams_manhole'
  },
  ams:  {}
...
def detect_network_profile(network_table_names)
  counts = { cams: 0, wams: 0, ams: 0 }

  network_table_names.each_key do |name|
...
def standard_table_for_profile?(table_name, profile)
  config = NETWORK_PROFILES[profile]
  return false unless table_name.start_with?(config[:prefix])
  return false if table_name.start_with?(config[:user_defined_prefix])

  true
end

def build_asset_type_aliases(table_descriptions, network_table_names, profile)
  aliases = {}
  config  = NETWORK_PROFILES[profile]

  network_table_names.each_key do |table_name|
    next unless standard_table_for_profile?(table_name, profile)

    aliases[table_name.downcase] = table_name
    desc = table_descriptions[table_name].to_s.strip
    aliases[desc.downcase] = table_name unless desc.empty?
  end

  LEGACY_ASSET_TYPE_ALIASES_BY_PROFILE.fetch(profile, {}).each do |label, table|
    aliases[label.downcase] = table
  end
...
def asset_match_values(asset, asset_table, profile)
  ...
  pipe_table = NETWORK_PROFILES[profile][:pipe_table]
  if pipe_table && asset_table == pipe_table
...
def user_defined_survey_table?(table_name, description, profile)
  prefix = NETWORK_PROFILES[profile][:user_defined_prefix]
  return false unless table_name.start_with?(prefix)

  table_name.downcase.include?('survey') ||
    description.to_s.downcase.include?('survey')
end

def discover_survey_tables(network_table_names, table_descriptions, profile)
  known_tables = KNOWN_SURVEY_TABLES_BY_PROFILE.fetch(profile, [])

  from_known = known_tables.select { |name| network_table_names.key?(name) }

  from_suffix = network_table_names.keys.select do |name|
    standard_table_for_profile?(name, profile) &&
      name.end_with?('_survey') &&
      !from_known.include?(name)
  end

  from_user_defined = network_table_names.keys.select do |name|
    user_defined_survey_table?(name, table_descriptions[name], profile)
  end

  tables = (from_known + from_suffix + from_user_defined).uniq.sort
  stats  = {
    known: from_known.size,
    suffix: from_suffix.size,
    user_defined: from_user_defined.size
  }
...
def relationship_name_for_survey(survey_table, profile)
  config = NETWORK_PROFILES[profile]
  return nil if survey_table.start_with?(config[:user_defined_prefix])

  base = survey_table.sub(/^#{Regexp.escape(config[:prefix])}/, '').sub(/_survey$/, '')
  "#{base}_surveys"
end
...
def surveys_from_asset_relationship(asset, survey_table, profile)
  rel = relationship_name_for_survey(survey_table, profile)
  return [] if rel.nil?
...
def asset_type_matches_table?(survey_type, asset_table, table_descriptions, asset_type_aliases)
  ...
  mapped = asset_type_aliases[normalized.downcase]
  mapped == asset_table
end
...
def survey_matches_asset?(survey, asset, asset_table, survey_fields, table_descriptions,
                          profile, asset_type_aliases)
  match_values = asset_match_values(asset, asset_table, profile)
  flood_table  = NETWORK_PROFILES[profile][:flood_defence_survey]

  if flood_table && survey.table_info.name == flood_table
    raw_type = survey.user_text_40.to_s.strip
    asset_id = survey.user_text_39.to_s.strip
    mapped   = asset_type_aliases[raw_type.downcase]
  ...
  if survey_fields.key?('asset_type') && survey_fields.key?('asset_id')
    return true if asset_type_matches_table?(survey.asset_type, asset_table, table_descriptions, asset_type_aliases) &&
...
def survey_blank_asset_type_with_asset_id?(survey, asset, asset_table, survey_fields, profile)
  return false unless survey_fields.key?('asset_type') && survey_fields.key?('asset_id')
  return false if non_blank?(survey.asset_type)

  survey_asset_id_matches?(survey.asset_id, asset_match_values(asset, asset_table, profile))
end

# ---------------------------------------------------------------------------
# Discover network tables and profile
# ---------------------------------------------------------------------------
...
network_profile     = detect_network_profile(network_table_names)
asset_type_aliases  = build_asset_type_aliases(table_descriptions, network_table_names, network_profile)

puts "=== Network profile ==="
puts "Detected profile : #{NETWORK_PROFILES[network_profile][:label]}"
puts ''
...
survey_tables, survey_discovery_stats = discover_survey_tables(
  network_table_names, table_descriptions, network_profile
)

if survey_tables.empty?
  WSApplication.message_box(
    "No survey tables were found in the current #{NETWORK_PROFILES[network_profile][:label]} network.",
    'OK', '!', false
  )
...
puts '=== Survey discovery ==='
puts "Survey tables    : #{survey_tables.size} (#{survey_discovery_stats[:known]} known, " \
     "#{survey_discovery_stats[:suffix]} suffix, #{survey_discovery_stats[:user_defined]} user-defined)"
puts ''
...
val = WSApplication.prompt(
  'Reschedule Survey from Asset Selection (Multi-Network)',
  [
...
if asset_table.nil?
  WSApplication.message_box('Could not resolve the selected asset object type.', 'OK', '!', false)
  puts 'Script cancelled - unrecognised asset type selection.'
  exit
end
...
asset_profile  = table_profile(asset_table)
survey_profile = table_profile(survey_table)
if asset_profile && survey_profile && asset_profile != survey_profile
  WSApplication.message_box(
    "Selected asset table '#{asset_table}' and survey table '#{survey_table}' belong to different network types.\n\n" \
    'Choose tables from the same network (CAMS, WAMS, or AMS).',
    'OK', '!', false
  )
  puts 'Script cancelled - asset and survey table network type mismatch.'
  exit
end
...
user_defined_survey = survey_table.start_with?(NETWORK_PROFILES[network_profile][:user_defined_prefix])
...
puts "Relationship pass: #{user_defined_survey ? 'disabled (user-defined survey)' : 'enabled (standard survey)'}"
puts ''
...
selected_assets.each do |asset|
  ...
  rel_surveys = surveys_from_asset_relationship(asset, survey_table, network_profile)
...
net.row_objects(survey_table).each do |survey|
  selected_assets.each do |asset|
    ...
    if survey_matches_asset?(survey, asset, asset_table, survey_field_map, table_descriptions,
                             network_profile, asset_type_aliases)
      surveys_by_asset[asset_key][survey.id.to_s] = survey
    end

    if survey_blank_asset_type_with_asset_id?(survey, asset, asset_table, survey_field_map, network_profile)
